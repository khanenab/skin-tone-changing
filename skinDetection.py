import numpy as np
import cv2
from skinAreaDetection import get_skin_color
from otherSkinAreaDetection import new_skin_color
...
def change_skin(image_file, want_color1):
	...
	if skinMask is not None:
		...
		# Use a default mask covering the entire image
		skinMaskInv = np.ones_like(converted[:, :, 0], dtype=np.uint8) * 255
